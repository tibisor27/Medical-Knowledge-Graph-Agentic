...
from src.agent.state import ResolvedEntity, ConversationAnalysis
...
def extract_nutrients_from_results(graph_results: List[Dict[str, Any]]) -> Set[str]:
    """
    Extrage nutrienții descoperiți din rezultatele bazei de date.

    Caută în diverse structuri de rezultate:
    - medication_info.depleted_nutrients
    - validation.nutrients_to_recommend
    - nutrient_name, nutrient, depleted_nutrient
    """
    ...
    for result in graph_results:
        ...
        # MEDICATION_LOOKUP: medication_info.depleted_nutrients
        med_info = result.get("medication_info")
        if isinstance(med_info, dict):
            depleted = med_info.get("depleted_nutrients", [])
            if isinstance(depleted, list):
                for nut in depleted:
        ...
        # CONNECTION_VALIDATION: validation.nutrients_to_recommend
        validation = result.get("validation")
        if isinstance(validation, dict):
            nuts_to_rec = validation.get("nutrients_to_recommend", [])
            if isinstance(nuts_to_rec, list):
                for nut in nuts_to_rec:
                    if nut:
                        nutrients.add(nut)

        # Direct nutrient fields
        for key in ("nutrient_name", "nutrient", "depleted_nutrient"):
            value = result.get(key)
            if isinstance(value, str) and value:
                nutrients.add(value)

        # Nested depleted_nutrients array
        depleted = result.get("depleted_nutrients")
        if isinstance(depleted, list):
            for nut in depleted:
                if isinstance(nut, dict):
                    name = nut.get("name", "")
        ...
        # Any key containing "nutrient"
        for key, value in result.items():
            if "nutrient" in key.lower() and isinstance(value, str) and value:
                nutrients.add(value)
...
def print_state_debug(state):
    ...
    # 3. RESOLVED ENTITIES (de la entity_extractor)
    if state.get('resolved_entities'):
        print(f"\n----> RESOLVED ENTITIES ({len(state['resolved_entities'])}):")
        print(f"   {'ORIGINAL':<25} | {'RESOLVED (Neo4j)':<25} | {'TYPE':<12} | {'METHOD':<10} | {'SCORE'}")
        print("   " + "-"*95)
        for ent in state['resolved_entities']:
            ...
            method = ent.match_method if hasattr(ent, 'match_method') else ent.get('match_method', '')
            score = ent.match_score if hasattr(ent, 'match_score') else ent.get('match_score', 0)
            print(f"   {orig:<25} | {res:<25} | {typ:<12} | {method:<10} | {score:.2f}")
